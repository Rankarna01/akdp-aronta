...
<script>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.data.length === 0) {
                    htmlRows = `<tr><td colspan="5" class="text-center py-8 text-secondary"><i class="fa-solid fa-user-slash text-2xl block mb-2 opacity-50"></i> Tidak ada data penumpang ditemukan</td></tr>`;
                } else {
                    response.data.forEach(function(item) {
                        let iconJk = item.jenis_kelamin === 'Laki-laki' ? '<i class="fa-solid fa-mars text-blue-500 mr-1"></i>' : '<i class="fa-solid fa-venus text-pink-500 mr-1"></i>';
                        htmlRows += `
                            <tr class="hover:bg-gray-50/80 transition">
                                <td class="px-6 py-4 font-semibold text-gray-800">${item.nama}</td>
                                <td class="px-6 py-4 font-mono text-xs text-gray-600 bg-gray-50 inline-block my-3 px-2 py-1 rounded-lg border border-gray-100 ml-6">${item.nik}</td>
                                <td class="px-6 py-4">${iconJk} ${item.jenis_kelamin}</td>
                                <td class="px-6 py-4 font-medium text-gray-700">${item.no_hp}</td>
                                <td class="px-6 py-4 text-center space-x-1">
                                    ...
                                    <button onclick="deletePenumpang(${item.id})" class="text-danger hover:bg-danger/10 p-2 rounded-lg transition" title="Hapus"><i class="fa-solid fa-trash-can"></i></button>
                                </td>
                            </tr>
                        `;
                    });
    ...
    function renderPagination(meta) {
        ...
        if (meta.last_page > 1) {
            ...
            paginationHtml += `<button onclick="fetchPenumpang(${meta.current_page - 1}, currentSearch)" ${meta.current_page === 1 ? 'disabled class="px-2.5 py-1.5 rounded-lg text-gray-300 cursor-not-allowed"' : 'class="px-2.5 py-1.5 rounded-lg text-secondary hover:bg-gray-200 transition"'}>Sebelumnya</button>`;
            ...
            paginationHtml += `<button onclick="fetchPenumpang(${meta.current_page + 1}, currentSearch)" ${meta.current_page === meta.last_page ? 'disabled class="px-2.5 py-1.5 rounded-lg text-gray-300 cursor-not-allowed"' : 'class="px-2.5 py-1.5 rounded-lg text-secondary hover:bg-gray-200 transition"'}>Selanjutnya</button>`;
            paginationHtml += `</div>`;
        }
